...
from llm_config import AGENT_MODEL, with_quota_retry
...
# ---------------------------------------------------------------------------
# Same agent as agent_plain.py — Gemini replaces Claude.
# ---------------------------------------------------------------------------
...
_mcp_tools = asyncio.run(
    MultiServerMCPClient({
        "shopease": {
            # Use the active interpreter so the MCP subprocess sees the same venv packages.
...
def support_agent(user_input: str) -> str:
    ...
    def _run():
        ...
        reply = _normalize_content(result["messages"][-1].content)
        update_current_trace(output=reply)
        return reply

    return with_quota_retry(_run)
